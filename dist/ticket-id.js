...
exports.isConfirmationEmail = exports.extractTicketId = void 0;
/**
 ...
 */
function extractTicketId(emailContent) {
    if (!emailContent)
        return null;
...
exports.extractTicketId = extractTicketId;
/**
 * Usage example:
 *
 ...
 */
function isConfirmationEmail(emailSubject, emailBody) {
    ...
    const hasPassengers = /(?:number of passengers|travelers|guests)\s*:/i.test(body);
    // Check if Victoria Tours is mentioned
    const isVictoriaTours = body.includes("victoria tours");
    // Confirmation indicators in subject
    const subjectIndicates = subjectConfirmationPhrases.some((phrase) => subject.includes(phrase));
    // Confirmation indicators in body
    const bodyIndicates = bodyConfirmationPhrases.some((phrase) => body.includes(phrase));
    // Look for structural elements of a confirmation email
    const hasConfirmationStructure = hasTicketId &&
...
exports.isConfirmationEmail = isConfirmationEmail;
/**
 * Usage example:
 *
